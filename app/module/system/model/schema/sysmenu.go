...
func (SysMenu) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("menu_name").StructTag(`json:"menuName"`).Optional().Comment("菜单名称"),
		field.String("menu_title").StructTag(`json:"menuTitle"`).Optional().Comment("菜单标题"),
		field.String("menu_icon").StructTag(`json:"menuIcon"`).Optional().Comment("菜单图标"),
		field.String("condition").StructTag(`json:"condition"`).Optional().Comment("条件"),
		field.Int8("menu_type").StructTag(`json:"type"`).Optional().Comment("菜单类型(0目录,1菜单,2按钮)"),
		...
		field.Int64("model_id").StructTag(`json:"modelId"`).Optional().Comment("模型id"),
		...
		field.Int8("is_link").StructTag(`json:"isLink"`).Optional().Comment("是否是外部链接(1是,0否)"),
		field.Int8("is_cached").StructTag(`json:"isCached"`).Optional().Comment("是否缓存(1是,0否)"),
		field.Int8("is_affix").StructTag(`json:"isAffix"`).Optional().Comment("是否固定(1是,0否)"),
